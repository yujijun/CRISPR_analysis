...
for (i in seq(1,dim(input_LFC)[2])){
  rra_LFC <- input_LFC[,i]
  p <- rankplot(gene_list,rra_LFC = rra_LFC,rra_genename = rra_gene,figure_title = cohort[i])
...
#single gene(STAT1)
cohort <- colnames(input_LFC)
output_path <-"/Users/yujijun/Documents/01-Work/05-CRESPR_SCREEN/crisprproject1/CRISPR_output_plot/RankView_JAK1"
gene_list <-  "JAK1"
rra_gene <- rownames(input_LFC)

for (i in seq(1,dim(input_LFC)[2])){
  rra_LFC <- input_LFC[,i]
  p <- rankplot_singlegene(gene_list,rra_LFC = rra_LFC,rra_genename = rra_gene,figure_title = cohort[i])
  figure_output = paste(paste(cohort[i],"_","JAK1",sep = ""),".png",sep = "")
  png(paste(output_path,figure_output,sep = "/"),height = 10,width = 12,units ="cm",res=150)
  print(p)
  dev.off()
}
